...
| `IMP-046` | P1 | Partly done (fixtures + diagnostics) | `-- --debug-mode` mounts the panel (`gate:debug-start`). Named fixtures now cover open_corridor / return_ready / loot_delta / **combat_victory** (lands in the right scene via `_scene_for_phase`, `verify_debug_fixtures`). In-mode diagnostics added: the panel shows scene · phase · **state hash** · **focused control** · **recent named inputs** (locked by `gate:debug-start`). Remaining: `map_modal` / `shop_description` fixtures (need scene UI-state seeding) and action-trace replay — both coupled to the `IMP-054` fixture/observation harness. |
...
| `IMP-050` | P1 | Partly done (3 steps) | `slice_rules.gd` down from 1,235 → **~965** lines across three separate revertible commits, verify_parity byte-identical throughout (36/36): (1) **party_commands.gd** + shared **roster_util.gd**; (2) **item_commands.gd**; (3) shared **rules_util.gd** hoist of the turn-incrementing `log_only` (groundwork, de-dups the item copy). Remaining: the **crawl cluster** — move/turn/search/visit_room/stairs/return/enter/checkpoint + their grid/floor/cell-effect helpers — which is the tightly-coupled core (shared direction consts, and some helpers are shared with the combat verbs). It is a focused pass: analyse each helper's consumers, hoist the truly-shared ones (`_turn` + the direction consts) into rules_util, then extract, holding verify_parity 36/36 (b1f-turns/wall/exploration/shortcut/stairs/return, b2f-hazard, b4f-spinner/teleport, verdant-walk cover it). |
