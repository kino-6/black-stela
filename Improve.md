...
| --- | --- | --- | --- |
| `IMP-021` | High | Done | Visual acceptance passed on the Godot 生業 service (rendered `_ux_town-career.png`): a pinned この冒険者 pane (current vocation / mastery / loadout) beside bounded destination cards, each with authored ±stat chips, granted technique, equipment access and prerequisites — reads as a DRPG career screen, not a text catalog, and the selected adventurer + cancel stay visible as focus moves. |
| `IMP-022` | High | Done | Every remaining capability is implemented + tested: rich affix effects (hp/mp/resistBonus/elementResist + combat-applied `regen` at round top and `speciesBonus` damage vs a family — rulesEngine + economy, authored in verdant/affixes.md, `affixEffects.test`); controller-first bulk selection with a rarity filter→preview→confirm (`loot_panel.gd`); and ≥2 viable answers per dangerous family — `sim:content` reports `findings: []` (no thin-coverage flag) for both worlds. Appraiser/Forge are full two-pane town service scenes (visual acceptance, same bar as IMP-021). 18 affix/loot tests green. |
...
| `IMP-024` | P1 | Done | The enemy stage now places creatures in a reserved band (`combat.gd _enemy_band()`) that clears the command panel (right) and, when shown, the acting-member panel (left); the backdrop still fills the full frame (#15 intact). A 3-group fight — the case a single centred enemy masked — renders every creature clear of the HUD. Locked by `verify_combat_geometry.gd` (enemy-mark screen rects vs every HUD rect, gate:migration 26) + rendered evidence (`_combat_trio.png`). |
...
| `IMP-028` | P1 | Done | Visual acceptance passed on the Godot creation flow (rendered `_ux_guild-class.png` / `_ux_guild-bonus.png`): class is a bounded list + stable detail pane (role/formation/signature/techniques/aptitude/starting gear), aptitude is a compact status window (base/current/±/effect/remaining points + reroll), and a reserved portrait+derived-stats preview persists through every step — a focused adventurer-making sequence, not a card wall or web-form stepper. Locked by `verify_guild_controller`. |
...
| `IMP-041` | P3 | Done (plumbing) | Density is now scenario-authored: `world.balance.wanderingEncounterPct` / `wanderingCooldownSteps` (schema + type), read live by BOTH runtimes (`rulesEngine.beginWanderingEncounter`, Godot `encounters.gd`) with the engine constants as defaults; per-table `respawns` was already authored (#20). Omitted ⇒ identical behaviour, so `verify_parity` stays byte-identical (verdant-walk). Locked by `tests/wanderingDensity.test.ts`; documented in `default/world.md`. The first-contact-per-visit silence is confirmed intended; the actual density VALUES stay a playtest/feel call (no world overrides yet). |
...
| `IMP-045` | P1 | Done | The last gap — #3's loot-delta on the real scene — is now gate-driven: `verify_loot_delta.gd` loads the `loot_delta` fixture into the real town scene and asserts the 持ち帰った物 row reports the gained item (灯油) but NOT the untouched descent supply (治癒の水薬), which still shows under supplies. With #13 (Esc) and #17 (held repeat) already on the real dungeon scene via `verify_dungeon_controller`, the real-scene coverage is complete. |
| `IMP-046` | P1 | Done | `-- --debug-mode` mounts the panel (`gate:debug-start`). All six named QA starts exist — open_corridor / map_modal / combat_victory / return_ready / loot_delta / shop_description — via the shared harness (`verify_scene_harness`), map_modal/shop_description driving existing scene seams (`_toggle_full_map` / `set_ui_state`), no scene-logic change. In-mode diagnostics show scene · phase · state hash · focused control · recent named inputs (`gate:debug-start`). Action-trace replay reproduces the same state hash from the same fixture (`Harness.replay`, gated). |
| `IMP-047` | P1 | Done | The export-safe `_texture` pattern (prefer the imported `ResourceLoader` resource; raw `Image.load_from_file` only as a last resort) is in title.gd, town.gd AND dungeon.gd — no scene reads a raw path export strips. `gate:package-smoke` builds the real macOS export and now passes with 0 warnings / 0 SCRIPT ERRORs / 0 missing resources / 0 orphan `.uid` sidecars, so the shipped title/town/dungeon backdrops are proven present in the packaged artifact. |
...
| `IMP-049` | P1 | Done | `npm run gate:changed -- --scope <scope>` (`scripts/gate-changed.mjs` + versioned `godot/gates/change-impact.json`) runs the fast + affected + native checks a scope must pass, AUTO-ESCALATES to `gate:migration` when a high-risk file is in the diff / the scope declares a trigger / it is `alwaysFullRoute` (save), prints the selected contract (run, not-run, why) + a machine-readable handoff, and exits non-zero on unknown scope or any failed command. `--plan` previews without running. Locked by `tests/gateChanged.test.ts` (manifest integrity + runner behaviour, 9 tests). |
| `IMP-050` | P1 | Done | `slice_rules.gd` reduced **1,235 → 246 lines** across four separate revertible commits, verify_parity byte-identical throughout (36/36): **party_commands.gd** (+ shared **roster_util.gd**), **item_commands.gd**, shared **rules_util.gd** (`log_only`), and **exploration_commands.gd** — the whole crawl (turn/search/move + grid/floor/cell-effect/chamber-chest helpers, stairs/return/enter/checkpoint/disarm, 785 lines). slice_rules is now a thin dispatcher + the combat exits; every command family is a pure leaf module. Acceptance met: modules are pure (no Node/scene/input/fs), parity + save + focused rule tests unchanged, the dispatcher's commands and unknown-command no-op are byte-for-byte compatible. |
| `IMP-051` | P2 | Partly done (renderer) | `dungeon_renderer.gd` extracted (the SubViewport, palette environment, camera, torch, cell geometry) — a pure collaborator with no game-state authority; `dungeon.gd` 1,181 → 1,010 lines, gate:migration 26 + Verdant render identical. Remaining: the HUD collaborator (minimap/party rail/dock/modals — coupled to dispatch) and the input collaborator (held-repeat timing — coupled to modal/chest/busy stop-conditions); both are lower-value, higher-coupling extractions than the renderer. |
| `IMP-052` | P2 | Done | Three presentation collaborators extracted from `combat.gd`: **combat_stage.gd** (backdrop, acting-member figure, placement band, enemy marks + reticle), **combat_party_hud.gd** (the vitals slot), **combat_playback.gd** (damage/defeat flourishes). All are pure builders that RETURN Controls; the scene keeps the layer, caches (`_enemy_marks`/`_party_slots`) and target selection, and `CombatRound` stays the sole combat authority. combat-controller + combat-geometry green, renders (start/victory/trio) identical, gate:migration 26. |
| `IMP-053` | P1 | Done | `world_resources.gd` is the one runtime resource boundary — `read_json`, the export-safe `texture` (imported-first, raw only as last resort, per IMP-047), and `world_asset` (one world→default fallback rule). Title, town, guild, dungeon, combat and result all route their `_texture`/`_asset` through it, so the packaging rule and the fallback live in ONE place instead of six divergent copies. gate:migration 26 + package-smoke green (0 warnings/missing). |
| `IMP-054` | P1 | Done | `godot/tests/scene_harness.gd` is the one harness: named fixtures (open_corridor / map_modal / combat_victory / return_ready / loot_delta / shop_description) seed a deterministic state via `debug_fixtures`, then follow-ups travel through normal scene code (`set_ui_state` seams or an existing method like `_toggle_full_map`) — never new scene logic. `observe()` returns scene · phase · state hash · focus, `replay()` re-runs an action trace through `Run.dispatch`, and the harness rejects an unknown name. Locked by `verify_scene_harness.gd` (`gate:migration`, 25 checks). Feeds IMP-046's fixture deck. |
