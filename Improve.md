...
| `IMP-051` | P2 | Done | `dungeon.gd` reduced **1,181 → 918 lines** into a thin scene spine over three collaborators: **dungeon_renderer.gd** (SubViewport, palette environment, camera, torch, cell geometry), **dungeon_hud.gd** (the party-rail token builder), **dungeon_input.gd** (held-key auto-repeat). The scene keeps state acquisition, dispatch and scene transitions; collaborators are pure/self-contained and mutate no game state. Renders identical, `verify_dungeon_controller` (#13/#17/#29) + gate:migration 26 green. |
