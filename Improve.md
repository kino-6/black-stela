...
| `IMP-050` | P1 | Partly done (2/4 slices) | `slice_rules.gd` down from 1,235 → **972** lines: **party_commands.gd** (+ shared **roster_util.gd**) and **item_commands.gd** extracted VERBATIM, each a separate revertible commit, with verify_parity byte-identical (roster 9/9, growth-items 4/4, escape-resume 2/2) and gate:migration 24 green. Remaining slices — **exploration_commands** (move/turn/search/grid/cell-effects/visit_room) and **expedition_commands** (enter/stairs/return/checkpoint/continue) — are the deeply-entangled half; they should first hoist the shared `_log_only`/`_turn` into a rules util, then extract, holding verify_parity 36/36 (the exploration traces cover them well). |
