...
**✅ V1–V4 全完遂 + merged to main（`0e7d06e`、2026-08-05）:** terminal-line を実機（Godot build）で遊んで出た 4 件を
React+Godot 両建てで修正（base world はフォールバックで不変）。**V1** 第一入口名を別世界の「黒碑へ潜る」→「零番線へ降りる /
Descend to Platform Zero」。**V2** 作成/名簿/転職/サマリの職業名が世界の再スキン（戦士→保安隊員等）を反映（React
`localizedVocationName`/`getActiveWorld()` 経由、Godot `guild.gd`/`town.gd _class_label` が `world.vocations` 優先）。
**V3** 見繕い(quick-recruit)が `world.portraits` からポートレートを配る（React `createSuggestedRecruitForParty(world)`、
