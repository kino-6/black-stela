extends RefCounted
## The dungeon CRAWL command handlers, extracted VERBATIM from slice_rules (IMP-050): turn / search /
## move (+ the grid, floor, cell-effect and chamber-chest helpers), stairs / return / enter / checkpoint /
## disarm. Pure — each returns {state, events}; ordering, seeds, event text, turn increments and dict shape
## are unchanged, so the dispatcher's crawl cases just forward here. The exploration/expedition golden
## traces (b1f-turns/wall/exploration/shortcut/stairs/return, b2f-hazard, b4f-spinner/teleport, verdant-walk)
## prove byte-for-byte parity. Names keep their leading underscore; the dispatcher calls them by module.
