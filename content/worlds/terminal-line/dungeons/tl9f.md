...
edges:
  ...
  - { from: room.tl9f.counterweight-bypass, direction: north, kind: shortcut, to: room.tl9f.bypass-control }
...
rooms:
  ...
  - id: room.tl9f.counterweight-bypass
    ...
    description: A narrow maintenance bridge makes a risky but short loop around the lift hall.
    locales: { ja: { name: 釣合い錘の迂回路, description: 狭い保守橋は、昇降機広間を危うくも短く回り込める。 } }
    gates: [{ id: gate.tl9f.counterweight, kind: shortcut, grantsFlag: flag.tl9f.counterweight-open, clue: A counterweight brake opens a maintenance bypass. }]
  ...
  - { id: room.tl9f.cable-drum, name: Cable Drum, description: A cable drum hides a sealed cache below its rusted rim., locales: { ja: { name: ケーブルドラム, description: 錆びた縁の下に、ケーブルドラムが封じた保管箱を隠す。 } }, chest: { treasureTable: treasure.tl9f.lift-cache, trap: { kind: gas, difficulty: 26, damage: 10 } } }
