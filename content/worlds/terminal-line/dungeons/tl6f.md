...
rooms:
  ...
  - id: room.tl6f.archive-lift
    name: Archive Lift
    description: A paper lift holds a recovery record for a former service channel.
    locales: { ja: { name: 書類用リフト, description: 書類用リフトには、復旧した保守路の記録票が残る。 } }
    gates: [{ id: gate.tl6f.lift, kind: shortcut, grantsFlag: flag.tl6f.lift-online, clue: The archive lift logs a records-clearance. }]
  ...
  - { id: room.tl6f.records-vault, name: Records Vault, description: The deepest files name the central isolation bureau as the line's destination., locales: { ja: { name: 記録金庫, description: 最も深い記録は、中央隔離局こそ路線の行先だと示す。 } }, encounterTable: encounters.tl6f.records-vault, chamberGuardian: true }
