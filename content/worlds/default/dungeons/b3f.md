...
tags:
  ...
  - block-cap
authorNotes: >-
  Maze floor (棒倒し法 base + carved chambers, seed 30303), reusing B3F's rooms and
  gimmicks. The winch, the Cistern Crossing (玄室, bitter-mote), and the warrens sit as
  chambers off the winding galleries; dead-end niches hide caches and the silt vault
  (searched wall). Winding the drowned winch opens the drop-shaft shortcut at the Chain
  Descent (the down-stair) back to the entry — so the deep stair rewards a full sweep,
  and the return rides home only once the winch is found. Stairs climb west to B2F; the
  chain descent falls west to B4F. Generated by scripts/genFloorMaze.mjs.
...
rooms:
  ...
  - id: room.b3f.hub
    name: Cistern Crossing
    description: A wide central chamber where the cistern's four galleries meet under a slow drip. The party keeps crossing back through it.
    locales:
      ja:
        name: 竪坑の踊り場
        description: 蓋をした竪坑の上の、手すり付きの踊り場。中央の辻で、四方の道が交わる。
    encounterTable: encounters.b3f.cistern
  - id: room.b3f.006
    name: Sunken Warren
