...
/** A permanent-on-wear modifier. It is resolved by the same effective-stat pipeline as equipment. */
export interface PassiveTechniqueBonus {
  attack?: number;
  armor?: number;
  accuracy?: number;
  speed?: number;
...
export interface Technique {
  ...
  tags?: string[];
  /** Present only for `kind: passive`; active techniques always resolve through `effects`. */
  passiveBonus?: PassiveTechniqueBonus;
}
...
export function validateTechnique(technique: Technique): string[] {
  ...
  if (technique.kind === "passive") {
    if (technique.effects.length !== 0) {
      problems.push(`${technique.id}: a passive resolves through passiveBonus, not combat effects`);
    }
    if (!technique.passiveBonus || Object.keys(technique.passiveBonus).length === 0) {
      problems.push(`${technique.id}: a passive needs a stat or resistance bonus`);
    }
    if (Object.keys(technique.cost).length !== 0) {
      problems.push(`${technique.id}: a passive cannot spend a combat resource`);
    }
    if (technique.duration.kind !== "instant") {
      problems.push(`${technique.id}: a passive persists only while its equipment is worn`);
    }
    return problems;
  }
  if (technique.effects.length === 0) {
