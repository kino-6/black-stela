...
Status: **W1受入待ち / 未登録**。ユーザーが `terminal-line` を指定して実装準備を指示したため、表示名と
world id はこの文書の値で運用確定する。Claude の受入済み canonical data が届くまで、W1へ渡す
...
世界選択への固定とW0完了扱いは、Claudeの受入済みF1/F2 dataが揃ってから行う。
...
- [x] 表示名「終端隔離線 — 零番線」を採用する（ユーザーの `terminal-line` 実装指定、2026-08-03）。
- [x] `terminal-line` を正式 world id とする（同上）。
