# Completed Task Archive Index

Archived: 2026-07-05

Active state: [../../Plan.md](../../Plan.md) (Tasks.md was archived 2026-07-20 as Tasks.react-era-combat-feel.md)

## Completed Task Archives

- [x] BS-001..BS-040:
  [Tasks.completed-modernization.md](Tasks.completed-modernization.md)
- [x] BS-041..BS-061:
  [Tasks.completed-scenario-tempo.md](Tasks.completed-scenario-tempo.md)
- [x] BS-062, BS-063, BS-068..BS-070, BS-072, BS-073:
  [Tasks.completed-play-parity-gate.md](Tasks.completed-play-parity-gate.md)
- [x] BS-074..BS-084:
  [Tasks.completed-tactical-combat.md](Tasks.completed-tactical-combat.md)
- [x] BS-067:
  [Tasks.completed-character-creation-planning.md](Tasks.completed-character-creation-planning.md)
- [x] BS-097:
  [Tasks.completed-reachability-terminology.md](Tasks.completed-reachability-terminology.md)
- [x] BS-064..BS-066, BS-071, BS-085..BS-096:
  [Tasks.completed-guild-registration.md](Tasks.completed-guild-registration.md)
- [x] BS-098..BS-103:
  [Tasks.completed-drpg-cockpit-ui.md](Tasks.completed-drpg-cockpit-ui.md)
- [x] BS-104..BS-113:
  [Tasks.completed-playable-depth.md](Tasks.completed-playable-depth.md)
- [x] BS-123:
  [Tasks.completed-stair-progression-gate.md](Tasks.completed-stair-progression-gate.md)
- [x] BS-114..BS-122, BS-124..BS-127:
  [Tasks.completed-character-authoring-scenario-authoring.md](Tasks.completed-character-authoring-scenario-authoring.md)
- [x] BS-136..BS-140:
  [Tasks.completed-drpg-ux-formation.md](Tasks.completed-drpg-ux-formation.md)
- [x] BS-128..BS-135, BS-141..BS-144:
  [Tasks.completed-grid-prose-command.md](Tasks.completed-grid-prose-command.md)
- [x] BS-145..BS-148:
  [Tasks.completed-view-map-parity.md](Tasks.completed-view-map-parity.md)
- [x] BS-149..BS-154:
  [Tasks.completed-guild-flow-reconstruction.md](Tasks.completed-guild-flow-reconstruction.md)
- [x] BS-155:
  [Tasks.completed-controller-first-contract.md](Tasks.completed-controller-first-contract.md)
- [x] BS-159..BS-162:
  [Tasks.completed-equipment-depth.md](Tasks.completed-equipment-depth.md)
- [x] BS-163..BS-166:
  [Tasks.completed-party-round-combat-depth.md](Tasks.completed-party-round-combat-depth.md)
- [x] BS-167..BS-170:
  [Tasks.completed-sequential-party-command-entry.md](Tasks.completed-sequential-party-command-entry.md)
- [x] BS-171..BS-176:
  [Tasks.completed-combat-cockpit-map-repair.md](Tasks.completed-combat-cockpit-map-repair.md)
- [x] BS-156..BS-158:
  [Tasks.completed-controller-first-normal-play-ui.md](Tasks.completed-controller-first-normal-play-ui.md)
- [x] BS-177..BS-182:
  [Tasks.completed-browser-selfplay-gate.md](Tasks.completed-browser-selfplay-gate.md)
- [x] BS-183..BS-188:
  [Tasks.completed-town-return-loop.md](Tasks.completed-town-return-loop.md)
- [x] BS-189..BS-193:
  [Tasks.completed-text-externalization-line-layout.md](Tasks.completed-text-externalization-line-layout.md)
- [x] BS-194..BS-197:
  [Tasks.completed-starting-gear-categorized-shop.md](Tasks.completed-starting-gear-categorized-shop.md)

## Plan To Task Traceability

| Plan Lane | Task Coverage | Status |
|---|---|---|
| Lane D2: Character Authorship Recovery | BS-114..BS-120 archived | Completed current recovery pass |
| Lane F: Scenario Authoring and Content QA | BS-121..BS-127 archived | Completed current authoring QA pass |
| Lane A: DRPG Presentation and UX | BS-064..BS-066, BS-098..BS-103 archived | Completed current presentation gate |
| Lane B: Honest Simulation and Play Parity | BS-070, BS-071, BS-097 archived | Completed current play-parity gate |
| Lane C: Human Requirement Gate | BS-071, BS-096 plus gate docs archived | Completed current gate coverage |
| Lane D: Character Creation and Roster Identity | BS-085..BS-096 archived | Superseded by D2 recovery pass |
| Lane E: Playable Depth | BS-104..BS-113 archived | Completed current playable-depth slice |
| Lane E2: Tactical DRPG Combat | BS-074..BS-084 archived | Completed vertical slice |
| Lane J: Grid Labyrinth Topology Recovery | BS-128..BS-135 archived | Completed current grid pass |
| Lane F2: Scenario Prose and Localization Quality | BS-141..BS-142 archived | Completed current prose gate pass |
| Lane I: Controller-First Command UI Reconstruction | BS-143..BS-144 archived | Completed current command gate pass |
| Lane K: DRPG UX Autonomy and Six-Person Formation | BS-136..BS-140 archived | Completed current pass |
| Lane M: First-Person View and Minimap Parity | BS-145..BS-148 archived | Completed current view/map parity pass |
| Lane N: Guild Registration Flow Reconstruction | BS-149..BS-154 archived | Completed current guild flow pass |
| Lane O: Controller-First Normal-Play UI | BS-155 archived; BS-156..BS-158 archived | Completed current controller-first slice |
| Lane P: DRPG Equipment Depth | BS-159..BS-162 archived | Completed current equipment-depth pass |
| Lane Q: Party-Round Combat Depth | BS-163..BS-166 archived | Completed current combat-depth pass |
| Lane R: Sequential Party Command Entry | BS-167..BS-170 archived | Completed current combat-command pass |
| Lane S: Combat Cockpit and Map Presentation Repair | BS-171..BS-176 archived | Completed current combat/minimap repair slice |
| Lane T: Browser Self-Play Gate | BS-177..BS-182 archived | Completed current browser self-play slice |
| Lane V: Scenario Text Externalization and Japanese Line Layout | BS-189..BS-193 archived | Completed event-text externalization and Japanese line-layout gate |
| Lane W: Starting Gear, Categorized Shop, and Pre-Purchase Preview | BS-194..BS-197 archived | Completed starting-gear equipping, categorized shop, and registration gear display |
| Lane U: Town Return Loop and Japanese DRPG Service UX | BS-183..BS-188 archived | Completed current town-loop slice |
| Lane Y: Guild Roster Management / Lifecycle / Cross-Scenario Adventurers | slices A–C archived (Plan.completed-guild-roster-lifecycle.md) | Completed roster/reclass/retire/vault |
| Difficulty Pressure, Maze Rollout, Playability & App Decomposition | commits f3f9d77..52cb3c6 archived (Plan.completed-difficulty-maze-decomposition.md) | Completed; balance tuning deferred |
| Lane G: Desktop Productization | BS-034..BS-037 archived; no active new tasks | Deferred |
| Lane H: Hidden Local Narration Operations | BS-019..BS-024 archived; no active new tasks | Deferred |
